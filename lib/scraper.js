...
exports.getURCharacterPages = exports.extractCharacterData = exports.fetchFromWeb = exports.closeBrowserFetcher = exports.getDokkanData = void 0;
...
let browserFetcherPromise;
let currentBrowserFetcher;
...
async function getDokkanData(rarity) {
    const document = await fetchFromWeb(`${CATEGORY_URL}${rarity}`);
    ...
    return charactersData.filter((character) => character != null);
}
...
async function waitForRealContent(page, strategyDescription) {
...
function getBrowserStrategies() {
    ...
    if (process.env.DOKKAN_ALLOW_HEADED_BROWSER === '1') {
        strategies.push({ channel: 'msedge', headless: false, description: 'Edge headed' }, { channel: 'chrome', headless: false, description: 'Chrome headed' }, { headless: false, description: 'Chromium headed' });
    }
    return strategies;
}
...
async function retireBrowserFetcher(browserFetcher) {
...
async function closeRetiredBrowserFetcher(browserFetcher) {
...
async function closeBrowserFetcher() {
...
exports.closeBrowserFetcher = closeBrowserFetcher;
async function createBrowserFetcher(strategyIndex) {
    let playwright;
    try {
...
async function buildRequestHeaders(url) {
...
async function getBrowserCookieHeader(url) {
    if (!browserFetcherPromise) {
        return undefined;
    }
    try {
...
async function fetchFromWeb(url) {
    ...
    const dom = new jsdom_1.JSDOM(HTMLData, { url });
...
function extractCardImageUrl(root) {
    if (!root) {
        return undefined;
    }
    const candidates = [];
...
function extractClassAndType(root) {
    ...
    return {
        class: classMatch ? character_1.Classes[classMatch[1]] : undefined,
        type: typeMatch ? character_1.Types[typeMatch[1]] : undefined,
    };
}
function extractCharacterData(characterDocument) {
    ...
    const characterData = {
        ...
        rarity: character_1.Rarities[baseTable?.querySelector('tbody > tr:nth-child(3) > td:nth-child(3) > center a')?.getAttribute('title')?.split('Category:')[1] ?? 'Error'],
        class: classAndType.class ?? character_1.Classes[baseTable?.querySelector('tbody > tr:nth-child(3) > td:nth-child(4) > center:nth-child(1) > span > a:nth-child(1)')?.getAttribute('title')?.split(' ')[0].split('Category:')[1] ?? 'Error'],
        type: classAndType.type ?? character_1.Types[baseTable?.querySelector('tbody > tr:nth-child(3) > td:nth-child(4) > center:nth-child(1) > span > a:nth-child(1)')?.getAttribute('title')?.split(' ')[1] ?? 'Error'],
...
function extractTransformedCharacterData(characterDocument) {
    ...
    for (let index = 1; index < transformCount; index++) {
        ...
        const transformationData = {
            ...
            transformedClass: classAndType.class ?? character_1.Classes[transformationTable?.querySelector('tbody > tr:nth-child(3) > td:nth-child(4) > center > a')?.getAttribute('title')?.split(' ')[0].split('Category:')[1] ?? 'Error'],
            transformedType: classAndType.type ?? character_1.Types[transformationTable?.querySelector('tbody > tr:nth-child(3) > td:nth-child(4) > center > a')?.getAttribute('title')?.split(' ')[1] ?? 'Error'],
...
async function getURCharacterPages() {
    let document = await fetchFromWeb(`${CATEGORY_URL}UR`);
    const ret = ['UR'];
    while (true) {
        ...
        const nextPage = button.href.split('/wiki/Category:')[1];
        if (!nextPage) {
            break;
        }
        ret.push(nextPage);
...
exports.getURCharacterPages = getURCharacterPages;
